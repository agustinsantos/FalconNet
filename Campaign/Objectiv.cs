...
namespace FalconNet.Campaign
{
	...
	gObjectiveCount++;
        ...
        public override int Wake()
        {
            ...
            {
                SetAwake(1);
                SimDriver.WakeCampaignFlight(false, this, GetComponents());
            }
            else
            {
                return 0;
	...
	//MonoPrint("%d (%d,%d) took %d losses from %d (%d,%d). range = %d\n",GetCampID(),tx,ty,losses,shooter.GetCampID(),sx,sy,range);
            ...
            CampWeaponsFireStatic.gDamageStatusPtr = addcrater; CampWeaponsFireStatic.gDamageStatusPtr++;

            // Record the final state, to keep remote entities consitant
            size = (byte)((static_data.class_data.Features + 3) / 4);
            CampWeaponsFireStatic.gDamageStatusPtr = size; CampWeaponsFireStatic.gDamageStatusPtr++;
            memcpy(CampWeaponsFireStatic.gDamageStatusPtr, obj_data.fstatus, size); CampWeaponsFireStatic.gDamageStatusPtr += size;
            ...
            while (*str > 0 && count < AIInput.MAX_DAMAGE_TRIES)
            {
                ...
                if (fc != null && fc.DamageMod[(int)d] > 0)
                {
                    ...
                    if (this_pass != null)
                    {
                        ...
                        CampWeaponsFireStatic.gDamageStatusPtr = (byte)f; CampWeaponsFireStatic.gDamageStatusPtr++;
                        CampWeaponsFireStatic.gDamageStatusPtr = (byte)s; CampWeaponsFireStatic.gDamageStatusPtr++;
        ...
        public virtual int DecodeDamageData(byte[] data, Unit shooter, FalconDeathMessage dtm)
        {
            ...
            for (i = 0; i < lost; i++)
            {
                ...
                if (islocal)
                    SetFeatureStatus(f, s);
                // Add runway craters
                if (EntityDB.Falcon4ClassTable[GetFeatureID(f)].vuClassData.classInfo_[VU_TYPE] == TYPE_RUNWAY) // (IS_RUNWAY)
                {
	...
	link_data = (CampObjectiveLinkDataType *)MemAllocPtr(gObjMemPool, sizeof(CampObjectiveLinkDataType)*static_data.links, 0 );
        ...
        {
            if (GetFalconType() == Classtable_Types.TYPE_CITY && obj_data.priority > PRIMARY_OBJ_PRIORITY)
                return 1;
            return 0;
        }
        ...
        public int IsSecondary()
        {
            ...
            if ((GetFalconType() == Classtable_Types.TYPE_CITY || GetFalconType() == Classtable_Types.TYPE_TOWN) && obj_data.priority > SECONDARY_OBJ_PRIORITY)
                return 1;
            return 0;
        }
	...
	obj_data.fstatus = (byte *)MemAllocPtr(gObjMemPool, sizeof(byte)*nsize, 0 );
        ...
        {
            Debug.Assert(static_data.class_data);
