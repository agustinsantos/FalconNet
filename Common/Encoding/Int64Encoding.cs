...
namespace FalconNet.Common.Encoding
{
    ...
    public class Int64EncodingLE
    {
        ...
        /// </returns>
        public static byte[] Encode(Int64 val)
        {
        ...
        /// </returns>
        public static Int64 Decode(byte[] buffer)
        {
        ...
        public static Int64 Decode(byte[] buffer, int startPos)
        {
            Debug.Assert(buffer != null, "Buffer is null");
            Debug.Assert(buffer.Length >= Size, "length = " + buffer.Length);
            if (EncodingHelpers.Endian != EndianTypes.LittleEndian)
