...
namespace FalconNet.FalcLib
{
     ...
     * Message Type ATCMsg
     */
    public class FalconATCMessage : FalconEvent
    {
        public enum ATCMsgCode
        {
            RequestClearance,
        ...
        public FalconATCMessage(VU_ID entityId, VuTargetEntity target, VU_BOOL loopback = true)
            : base((VU_MSG_TYPE)FalconMsgID.ATCMsg, HandlingThread.SimThread, entityId, target, loopback)
        {
            //me123 RequestOutOfBandTransmit ();
            RequestReliableTransmit(); //me123
        }
        public FalconATCMessage(VU_MSG_TYPE type, VU_ID senderid, VU_ID target)
            : base((VU_MSG_TYPE)FalconMsgID.ATCMsg, HandlingThread.SimThread, senderid, target)
        {
            // Your Code Goes Here
        }
        //public ~FalconATCMessage(void);
#if TODO
    ...
    public int Encode(VU_BYTE **buf)
    {
        ...
        *buf += sizeof(dataBlock);
        ...
        {
#if TODO
            AircraftClass aircraft = (AircraftClass)VUSTATIC.vuDatabase.Find(dataBlock.from);
            ObjectiveClass atc = (ObjectiveClass)VUSTATIC.vuDatabase.Find(EntityId());
            ...
            if (aircraft && aircraft.IsAirplane())
            {
                ...
                {
                    case ATCMsgCode.ContactApproach:
                    case ATCMsgCode.RequestClearance:
                    ...
                    case ATCMsgCode.RequestEmerClearance:
                        if (!aircraft.IsPlayer() || !aircraft.IsLocal())
                            SendCallToATC(aircraft, EntityId(), rcLANDCLEAREMERGENCY, FalconLocalSession);
                    ...
                    case ATCMsgCode.RequestTakeoff:
                        if (!aircraft.IsPlayer() || !aircraft.IsLocal())
                ...
                if (atcBrain && atc.IsLocal())
                {
                    //SendCallFromAwacs((Flight)aircraft.GetCampaignObject(), rcNOTASKING , static_cast<VuTargetEntity*>(VUSTATIC.vuDatabase.Find(this.Sender())));
                    SendCallFromATC(atc, aircraft, rcOUTSIDEAIRSPEED, static_cast<VuTargetEntity*>(VUSTATIC.vuDatabase.Find(aircraft.OwnerId())));
                    //SendCallToATC(aircraft, atc.Id(), rcREADYFORDERARTURE, static_cast<VuTargetEntity*>(VUSTATIC.vuDatabase.Find(aircraft.OwnerId())));
                }
                    ...
                    case ATCMsgCode.RequestHotpitRefuel:
                        if (!aircraft.IsPlayer() || !aircraft.IsLocal())
                            SendCallToATC(aircraft, EntityId(), rcCOPY, FalconLocalSession);
