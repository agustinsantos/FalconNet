...
namespace FalconNet.Graphics
{
    ...
    {
        public static int nVer; // Header version
    ...
    MemFreePtr(CompressedBuffer);
        ...
        public static void ReadPool(Stream file, string basename)
        {
            int result;
    ...
    CompressedBuffer = (BYTE *)MemAllocPtr(gBSPLibMemPool, sizeof(BYTE) * maxCompressedSize, 0);
            ...
            for (int i = 0; i < nTextures; i++)
                 TempTexBankEntryEncodingLE.Decode(file, ref TempTexturePool[i]);
            ...
            for (int i = 0; i < nTextures; i++)
            {
                ...
                if (DisplayOptionsClass.DisplayOptions.bMipmapping)
                {
    ...
    #region Encoding
    public static class TempTexBankEntryEncodingLE
    {
        public static void Encode(Stream stream, TempTexBankEntry val)
        {
            throw new NotImplementedException();
        }

        public static void Decode(Stream stream, ref TempTexBankEntry rst)
        {
                rst.fileOffset  = Int32EncodingLE.Decode(stream);
                rst.fileSize  = Int32EncodingLE.Decode(stream);
                rst.tex = new Texture();
                TextureEncodingLE.Decode(stream, rst.tex);
                rst.palID  = Int32EncodingLE.Decode(stream);
                rst.refCount  = Int32EncodingLE.Decode(stream);
        }

        public static int Size
        {
            get { return TextureEncodingLE.Size + 4 * Int32EncodingLE.Size; }
        }
    }
    #endregion
}
