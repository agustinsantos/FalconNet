...
namespace FalconNet.Graphics
{
    ...
    public class ThreeDVertex : TwoDVertex
    {
        ...
        public float csX, csY, csZ;		// Camera space coordinates
    }
    ...
    public class Render3D : Render2D
    {
        ...
        public override void Setup(ImageBuffer imageBuffer)
        {
            Tpoint pos = new Tpoint();

            horizontal_half_angle = (float)(Math.PI / 180.0f); // this is used in the 2d setup call
            detailScaler = 1.0f;
            SetFar(10000.0f);

            // Call our parents Setup code (win is available only after this call)
            base.Setup(imageBuffer);
            SetFOV((float)(60.0f * Math.PI / 180.0f));

            // Intialize our camera parameters to something reasonable

            pos.x = pos.y = pos.z = 0.0f;
            SetCamera(pos, Matrix.IMatrix);

            // Put a default light source into the 3D object drawing context
            lightAmbient = 0.5f;
            lightDiffuse = 0.5f;
            Tpoint dir = new Tpoint() { x = 0.0f, y = 0.0f, z = -1.0f };
        ...
        public override void StartFrame()
        {
            ...
            StateStackClass.TheStateStack.SetContext(context);
            StateStackClass.TheStateStack.SetCameraProperties(oneOVERtanHFOV, oneOVERtanVFOV, scaleX, scaleY, shiftX, shiftY);
            StateStackClass.TheStateStack.SetLight(lightAmbient, lightDiffuse, lightVector);
            StateStackClass.TheStateStack.SetLODBias(resRelativeScaler);
            StateStackClass.TheStateStack.SetTextureState(objTextureState);
        ...
        public void SetObjectDetail(float scaler)
        {
            ...
            const float RadiansPerPixel = (float)(60.0f * Math.PI / 180.0f) / 640;
        ...
        {
#if TODO
            const float maxHalfFOV = (float)(70 * Math.PI / 180.0f);
            ...
            const float minHalfFOV = (float)( (Math.PI / 180.0f) / 4);
            // JB 010120
            ...
            if (scaleX != 0)
            {
                vertical_half_angle = (float)Math.Atan2(scaleY * Math.Tan(horizontal_half_angle), scaleX);
                diagonal_half_angle = (float)Math.Atan2(Math.Sqrt(scaleX * scaleX + scaleY * scaleY) * Math.Tan(horizontal_half_angle), scaleX);
            }
            else
            {
                vertical_half_angle = (float)Math.Atan2(3.0f * Math.Tan(horizontal_half_angle), 4.0f);
                diagonal_half_angle = (float)Math.Atan2(5.0f * Math.Tan(horizontal_half_angle), 4.0f);
            }
            oneOVERtanHFOV = 1.0f / (float)Math.Tan(horizontal_half_angle);
            oneOVERtanVFOV = 1.0f / (float)Math.Tan(vertical_half_angle);
        ...
        public void SetCamera(Tpoint pos, Trotation rot)
        {
            ...
            MatrixTranspose(rot, ref cameraRot);

            // Back compute the roll, pitch, and yaw of the viewer
            pitch = (float)-Math.Asin(cameraRot.M13);
            roll = (float)Math.Atan2(cameraRot.M23, cameraRot.M33);
            yaw = (float)Math.Atan2(cameraRot.M12, cameraRot.M11);
            if (yaw < 0.0f) yaw += 2.0f * Math.PI;		// Convert from +/-180 to 0-360 degrees
            ...
            // Send relevant stuff to the BSP object library
            StateStackClass.TheStateStack.SetCamera(pos, &T, &Tbb, &Tt);
#endif
            throw new NotImplementedException();
        }
        ...
        public void GetAt(ref Tpoint v)
        {
            v.x = cameraRot.M11;
            v.y = cameraRot.M12;
            v.z = cameraRot.M13;
        ...
        public void GetLeft(ref Tpoint v)
        {
            v.x = cameraRot.M21;
            v.y = cameraRot.M22;
            v.z = cameraRot.M23;
        ...
        public void GetUp(ref Tpoint v)
        {
            v.x = cameraRot.M31;
            v.y = cameraRot.M32;
            v.z = cameraRot.M33;
        ...
        public void TransformPoint(Tpoint p, ref ThreeDVertex result)
        {
            float scratch_x;
            float scratch_y;
            float scratch_z;
            ClippingFlags clipFlag;
        ...
        public void TransformPointToView(Tpoint p, ref Tpoint result)
        {
            // This part does rotation, translation, and scaling
            // no swapping...
        ...
        public void TransformBillboardPoint(Tpoint p, Tpoint viewOffset, ref ThreeDVertex result)
        {
            float scratch_x;
            float scratch_y;
            float scratch_z;
        ...
        public void UnTransformPoint(Tpoint p, ref Tpoint result)
        {
        ...
        public void TransformCameraCentricPoint(Tpoint p, ref ThreeDVertex result)
        {
            float scratch_x;
            float scratch_y;
            float scratch_z;
        ...
        {
            ThreeDVertex ps1 = new ThreeDVertex();

            // Transform the point from world space to window space
            TransformPoint(p1, ref ps1);
            if (ps1.clipFlag != ClippingFlags.ON_SCREEN) return;
        ...
        {
            ThreeDVertex ps1 = new ThreeDVertex(), ps2 = new ThreeDVertex();

            // Transform the points from world space to window space
            TransformPoint(p1, ref ps1);
            TransformPoint(p2, ref ps2);

            // Quit now if both ends are clipped by the same edge
            if ((ps1.clipFlag & ps2.clipFlag) != 0) return;

            // Clip the line as necessary
            if ((ps1.clipFlag & ClippingFlags.CLIP_NEAR) != 0)
            {
                IntersectNear(ps1, ps2, ref ps1);
            }
            else if ((ps2.clipFlag & ClippingFlags.CLIP_NEAR) != 0)
            {
                IntersectNear(ps1, ps2, ref ps2);
            }
            if ((ps1.clipFlag & ps2.clipFlag) != 0) return;

            if ((ps1.clipFlag & ClippingFlags.CLIP_BOTTOM) != 0)
            {
                IntersectBottom(ps1, ps2, ref ps1);
            }
            else if ((ps2.clipFlag & ClippingFlags.CLIP_BOTTOM) != 0)
            {
                IntersectBottom(ps1, ps2, ref ps2);
            }
            if ((ps1.clipFlag & ClippingFlags.CLIP_TOP) != 0)
            {
                IntersectTop(ps1, ps2, ref ps1);
            }
            else if ((ps2.clipFlag & ClippingFlags.CLIP_TOP) != 0)
            {
                IntersectTop(ps1, ps2, ref ps2);
            }
            if ((ps1.clipFlag & ps2.clipFlag) != 0) return;

            if ((ps1.clipFlag & ClippingFlags.CLIP_RIGHT) != 0)
            {
                IntersectRight(ps1, ps2, ref ps1);
            }
            else if ((ps2.clipFlag & ClippingFlags.CLIP_RIGHT) != 0)
            {
                IntersectRight(ps1, ps2, ref ps2);
            }
            if ((ps1.clipFlag & ClippingFlags.CLIP_LEFT) != 0)
            {
                IntersectLeft(ps1, ps2, ref ps1);
            }
            else if ((ps2.clipFlag & ClippingFlags.CLIP_LEFT) != 0)
            {
                IntersectLeft(ps1, ps2, ref ps2);
            }
        ...
        {
            ThreeDVertex ps1 = new ThreeDVertex(), ps2 = new ThreeDVertex(), ps3 = new ThreeDVertex();

            // Transform the points from world space to window space
            TransformPoint(p1, ref ps1);
            TransformPoint(p2, ref ps2);
            TransformPoint(p3, ref ps3);
            ...
            if (ps1.clipFlag != 0 || ps2.clipFlag != 0 || ps3.clipFlag != 0) return;
        ...
        public void Render3DObject(int id, Tpoint pos, Trotation orientation)
        {
            ...
            StateStackClass.TheStateStack.DrawObject(&instance, orientation, pos);
#endif
            throw new NotImplementedException();
        }
        ...
        public void DrawSquare(ThreeDVertex v0, ThreeDVertex v1, ThreeDVertex v2, ThreeDVertex v3, int CullFlag, bool gifPicture = false)
        {
            ...
            if ((v0.clipFlag | v1.clipFlag | v2.clipFlag | v3.clipFlag) != 0)
            {
                ...
                if ((v0.clipFlag | v1.clipFlag | v2.clipFlag) != 0)
                {
                    vertPointers[0] = v0;
                    vertPointers[1] = v1;
                ...
                if ((v0.clipFlag | v2.clipFlag | v3.clipFlag) != 0)
                {
                    vertPointers[1] = v0;
                    vertPointers[3] = v3;
...
//		Debug.Assert ((*v).q >= -0.001F);
        ...
        private void InterpolateColorAndTex(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v, float t)
        {
        ...
        private void IntersectNear(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v)
        {
            float x, y, z, t;
        ...
        private void IntersectTop(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v)
        {
            float x, y, z, t;
            float dx, dy, dz;
        ...
        private void IntersectBottom(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v)
        {
            float x, y, z, t;
            float dx, dy, dz;
        ...
        private void IntersectLeft(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v)
        {
            float x, y, z, t;
            float dx, dy, dz;
        ...
        private void IntersectRight(ThreeDVertex v1, ThreeDVertex v2, ref ThreeDVertex v)
        {
            float x, y, z, t;
            float dx, dy, dz;
        ...
        public static ClippingFlags GetRangeClipFlags(float z, float d)
        {
            if (z < NEAR_CLIP)
            {
        ...
        public static ClippingFlags GetHorizontalClipFlags(float x, float z)
        {
            if (Math.Abs(x) > z)
            {
                if (x > z)
        ...
        public static ClippingFlags GetVerticalClipFlags(float y, float z)
        {
            if (Math.Abs(y) > z)
            {
                if (y > z)
