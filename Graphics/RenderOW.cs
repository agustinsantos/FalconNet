...
public  class RenderOTW : Render3D {
	...
	if (worldZ > -VALLEY_HAZE_TOP) {	// -Z is upward
...
// OW
	...
	public  const float		HAZE_ALTITUDE_FACTOR = (1.0f/(SKY_MAX_HEIGHT-SKY_ROOF_HEIGHT));
	public  const float		GLARE_FACTOR = (4096.0f/SKY_MAX_HEIGHT);

	public  Texture	texRoofTop;
	public  Texture	texRoofBottom;
...
//			Debug.Assert( northBoundry[n].end <= northBoundry[n-1].end );
	...
	protected virtual void ComputeVertexColor( TerrainVertex *vert, Tpost *post, float distance )
			{
	float	alpha;
	float	fog;
	float	inv_fog;
...
//		return;
	...
	// Debug.Assert( TheTimeOfDay.ThereIsASun() );
	...
	Debug.Assert( TheTimeOfDay.ThereIsAMoon() );
	...
	if (starblend > 0.000001f) {
 	...
 	protected virtual void ComputeHorizonEffect( HorizonRecord *pHorizon )
			{
	...
	if (!(sunEffectScreenSpace.clipFlag & CLIP_NEAR)) {
...
\***************************************************************************/
  ...
  const pnt[] OutsidePoints = new pnt[]{
	{	 ltl,	 big	},
	{	 1.0f,	 1.0f	},
	{	 big,	 ltl	},
	{	 big,	-ltl	},
  ...
  const int	NumPoints = sizeof(OutsidePoints)/sizeof(pnt);
  const float	PercentBlend = 0.1f;
  const float	PercentScale = 1.0f + PercentBlend;
}
