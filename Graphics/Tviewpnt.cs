...
public class TViewPoint {
	...
	DeleteCriticalSection( &cs_update );
  	...
  	public bool IsReady()		{ return (nLists != 0 ); }
	...
	{
		float dist = (float)sqrt(((pos.x - position.x)*(pos.x - position.x) + (pos.y - position.y)*(pos.y - position.y)));
	...
	public bool	GetArea( int TileID, int type, int offset, TareaFeature *target )
			{
	TexArea		*area;
	const int	LOD = TheMap.LastNearTexLOD();
	...
	public void	GetAreaFloorAndCeiling( float *floor, float *ceiling ){
	int		level;
	float	minZ;
	float	maxZ;
	...
	*ceiling = minZ;	// Remember, positive Z is downward in world space
	...
	public float	GetDrawingRange()		{ return LEVEL_POST_TO_WORLD( blockLists[lowDetail].GetAvailablePostRange(), lowDetail ) * 0.65f; }
	...
	public float	GetGroundLevel( float x, float y, Tpoint *normal=NULL );
	...
	private void LineSquareIntersection( int row, int col, Tpoint *dir, Tpoint *intersection, int LOD )
		{
	Tpost	SW, NW, NE, SE;
	float	Nx, Ny, Nz;
	float	SWx, SWy, SWz;
...
//	ShiAssert( (intersection.x-SWx) <= (intersection.y-SWy) );
	...
	protected TBlockList			*blockLists;
	protected CRITICAL_SECTION	cs_update;
