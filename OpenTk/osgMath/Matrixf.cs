using System;
using value_type = System.Single;
...
namespace OSGNet.Osg
{
    public class Matrixf
    {
        ...
        public Matrixf(Matrixf mat) { Set(mat.Data); }
        public Matrixf(Matrixd mat)
        {
            Set(mat.Data);
        }
        public Matrixf(float[,] ptr) { Set(ptr); }
        public Matrixf(double[,] ptr) { Set(ptr); }
        public Matrixf(Quat quat) { MakeRotate(quat); }

        public Matrixf(value_type a00, value_type a01, value_type a02, value_type a03,
        ...
        public static bool operator <(Matrixf l, Matrixf r) { return l.Compare(r) < 0; }
        public static bool operator >(Matrixf l, Matrixf r) { return l.Compare(r) > 0; }
        public static bool operator ==(Matrixf l, Matrixf r) { return l.Compare(r) == 0; }
        public static bool operator !=(Matrixf l, Matrixf r) { return l.Compare(r) != 0; }
        ...
        public bool IsNaN
        {
            get
            {
                return float.IsNaN(_mat[0, 0]) || float.IsNaN(_mat[0, 1]) || float.IsNaN(_mat[0, 2]) || float.IsNaN(_mat[0, 3]) ||
                               float.IsNaN(_mat[1, 0]) || float.IsNaN(_mat[1, 1]) || float.IsNaN(_mat[1, 2]) || float.IsNaN(_mat[1, 3]) ||
                               float.IsNaN(_mat[2, 0]) || float.IsNaN(_mat[2, 1]) || float.IsNaN(_mat[2, 2]) || float.IsNaN(_mat[2, 3]) ||
                               float.IsNaN(_mat[3, 0]) || float.IsNaN(_mat[3, 1]) || float.IsNaN(_mat[3, 2]) || float.IsNaN(_mat[3, 3]);
            }
        }
        ...
        public void Set(Matrixf rhs) { Set(rhs.Data); }

        public void Set(Matrixd rhs) { Set(rhs.Data); }
        ...
        {
            MakeScale((float)v[0], (float)v[1], (float)v[2]);
        }
        ...
        {
            MakeTranslate((float)v[0], (float)v[1], (float)v[2]);
        }
        ...
        public void Decompose(Vec3f translation,
                              Quat rotation,
                              Vec3f scale,
                              Quat so)
        {
        ...
        /** decompose the matrix into translation, rotation, scale and scale orientation.*/
        public void Decompose(Vec3d translation,
                         Quat rotation,
                         Vec3d scale,
                         Quat so)
        {
        ...
        */
                       ...
                       double zNear, double zFar)
        {
            ...
            SetRow(0, 2.0f / (float)(right - left), 0.0f, 0.0f, 0.0f);
            SetRow(1, 0.0f, 2.0f / (float)(top - bottom), 0.0f, 0.0f);
            SetRow(2, 0.0f, 0.0f, -2.0f / (float)(zFar - zNear), 0.0f);
            SetRow(3, (float)tx, (float)ty, (float)tz, 1.0f);
        }
        ...
        */
                         ...
                         double zNear, double zFar)
        {
            ...
            SetRow(0, (float)(2.0 * zNear / (right - left)), 0.0f, 0.0f, 0.0f);
            SetRow(1, 0.0f, (float)(2.0 * zNear / (top - bottom)), 0.0f, 0.0f);
            SetRow(2, (float)A, (float)B, (float)C, -1.0f);
            SetRow(3, 0.0f, 0.0f, (float)D, 0.0f);
        }
        ...
        */
                        ...
                        out double zNear, out  double zFar)
        {
            ...
            value_type temp_near = (float)(_mat[3, 2] / (_mat[2, 2] - 1.0));
            value_type temp_far = (float)(_mat[3, 2] / (1.0 + _mat[2, 2]));
        ...
        */
                            ...
                            out  double zNear, out  double zFar)
        {
            ...
            value_type top = 0.0f;
            value_type bottom = 0.0f;

            // note: near and far must be used inside this method instead of zNear and zFar
            // because zNear and zFar are references and they may point to the same variable.
            value_type temp_near = 0.0f;
            value_type temp_far = 0.0f;
        ...
        */
        public void MakeLookAt(Vec3d eye, Vec3d center, Vec3d up)
        {
            ...
            Set((float)s[0], (float)u[0], (float)-f[0], 0.0f,
        ...
        */
        ...
        public bool Invert(Matrixf rhs)
        {
        ...
        /// <returns></returns>
        public bool Invert_4x3(Matrixf mat)
        {
            if (mat == this)
            {
                Matrixf tm = new Matrixf(mat);
            ...
            value_type one_over_det = (float)(1.0 / (r00 * _mat[0, 0] + r10 * _mat[0, 1] + r20 * _mat[0, 2]));
        ...
        public bool Invert_4x4(Matrixf mat)
        {
            if (mat == this)
            {
                Matrixf tm = new Matrixf(mat);
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                if (irow != icol)
                    for (l = 0; l < 4; l++)
                    {
                        temp = this._mat[irow, l];
                        this._mat[irow, l] = this._mat[icol, l];
                ...
                for (l = 0; l < 4; l++) this._mat[icol, l] *= (float)pivinv;
                for (ll = 0; ll < 4; ll++)
                    if (ll != icol)
                    {
                        dum = this[ll, icol];
                        this._mat[ll, icol] = 0;
                        for (l = 0; l < 4; l++) this._mat[ll, l] -= (float)(this[icol, l] * dum);
                    }
            }
        ...
        public void OrthoNormalize(Matrixf rhs)
        {
            ...
            if (!Equivalent(x_colMag, 1.0f) && !Equivalent(x_colMag, 0.0f))
            {
                x_colMag = (float)Math.Sqrt(x_colMag);
            ...
            if (!Equivalent(y_colMag, 1.0f) && !Equivalent(y_colMag, 0.0f))
            {
                y_colMag = (float)Math.Sqrt(y_colMag);
            ...
            if (!Equivalent(z_colMag, 1.0f) && !Equivalent(z_colMag, 0.0f))
            {
                z_colMag = (float)Math.Sqrt(z_colMag);
        ...
        public static Matrixf Scale(Vec3f sv)
        {
            return Scale(sv.X, sv.Y, sv.Z);
        }
        public static Matrixf Scale(Vec3d sv)
        {
            return Scale((float)sv.X, (float)sv.Y, (float)sv.Z);
        }
        public static Matrixf Scale(value_type sx, value_type sy, value_type sz)
        {
            Matrixf m = new Matrixf();
            m.MakeScale(sx, sy, sz);
            return m;
        }
        public static Matrixf Translate(Vec3f v)
        {
            return Translate(v.X, v.Y, v.Z);
        }
        public static Matrixf Translate(Vec3d v)
        {
            return Translate((float)v.X, (float)v.Y, (float)v.Z);
        }
        public static Matrixf Translate(value_type tx, value_type ty, value_type tz)
        {
            Matrixf m = new Matrixf();
            m.MakeTranslate(tx, ty, tz);
            return m;
        }
        public static Matrixf Rotate(Vec3f from, Vec3f to)
        {
            Matrixf m = new Matrixf();
            m.MakeRotate(from, to);
            return m;
        }
        public static Matrixf Rotate(Vec3d from, Vec3d to)
        {
            Matrixf m = new Matrixf();
            m.MakeRotate(from, to);
            return m;
        }
        public static Matrixf Rotate(value_type angle, value_type x, value_type y, value_type z)
        {
            Matrixf m = new Matrixf();
        ...
        public static Matrixf Rotate(value_type angle, Vec3f axis)
        {
            Matrixf m = new Matrixf();
            m.MakeRotate(angle, axis);
            return m;
        }
        public static Matrixf Rotate(value_type angle, Vec3d axis)
        {
            Matrixf m = new Matrixf();
            m.MakeRotate(angle, axis);
            return m;
        ...
        public static Matrixf Rotate(value_type angle1, Vec3f axis1,
                                      value_type angle2, Vec3f axis2,
                                      value_type angle3, Vec3f axis3)
        {
            Matrixf m = new Matrixf();
        ...
        public static Matrixf Rotate(value_type angle1, Vec3d axis1,
                                      value_type angle2, Vec3d axis2,
                                      value_type angle3, Vec3d axis3)
        {
            Matrixf m = new Matrixf();
        ...
        public static Matrixf Rotate(Quat quat)
        {
            return new Matrixf(quat);
        }
        public static Matrixf Inverse(Matrixf matrix)
        {
            Matrixf m = new Matrixf();
            m.Invert(matrix);
            return m;
        }
        public static Matrixf OrthoNormal(Matrixf matrix)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create an orthographic projection matrix.
          * See glOrtho for further details.
        */
        public static Matrixf Ortho(double left, double right,
                                    double bottom, double top,
                                    double zNear, double zFar)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create a 2D orthographic projection.
          * See glOrtho for further details.
        */
        public static Matrixf Ortho2D(double left, double right,
                                      double bottom, double top)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create a perspective projection.
          * See glFrustum for further details.
        */
        public static Matrixf Frustum(double left, double right,
                                      double bottom, double top,
                                      double zNear, double zFar)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create a symmetrical perspective projection.
          ...
          * Aspect ratio is defined as width/height.
        */
        public static Matrixf Perspective(double fovy, double aspectRatio,
                                          double zNear, double zFar)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create the position and orientation as per a camera,
          * using the same convention as gluLookAt.
        */
        public static Matrixf LookAt(Vec3f eye,
                                       Vec3f center,
                                       Vec3f up)
        {
            Matrixf m = new Matrixf();
        ...
        /** Create the position and orientation as per a camera,
          * using the same convention as gluLookAt.
        */
        public static Matrixf LookAt(Vec3d eye,
                                       Vec3d center,
                                       Vec3d up)
        {
            Matrixf m = new Matrixf();
        ...
        public Vec3f PreMult(Vec3f v)
        {
            ...
            return new Vec3f((_mat[0, 0] * v.X + _mat[1, 0] * v.Y + _mat[2, 0] * v.Z + _mat[3, 0]) * d,
        ...
        {
            value_type d = (float)(1.0 / (_mat[0, 3] * v.X + _mat[1, 3] * v.Y + _mat[2, 3] * v.Z + _mat[3, 3]));
        ...
        public Vec3f PostMult(Vec3f v)
        {
            ...
            return new Vec3f((_mat[0, 0] * v.X + _mat[0, 1] * v.Y + _mat[0, 2] * v.Z + _mat[0, 3]) * d,
        ...
        {
            value_type d = (float)(1.0 / (_mat[3, 0] * v.X + _mat[3, 1] * v.Y + _mat[3, 2] * v.Z + _mat[3, 3]));
        ...
        {
            return new Vec4f((_mat[0, 0] * v.X + _mat[1, 0] * v.Y + _mat[2, 0] * v.Z + _mat[3, 0] * v.W),
        ...
        {
            return new Vec4f((_mat[0, 0] * v.X + _mat[0, 1] * v.Y + _mat[0, 2] * v.Z + _mat[0, 3] * v.W),
        ...
        public static Vec3f operator *(Vec3f v, Matrixf m)
        {
            return m.PreMult(v);
        }
        public static Vec3d operator *(Vec3d v, Matrixf m)
        {
            return m.PreMult(v);
        }
        public static Vec4f operator *(Vec4f v, Matrixf m)
        {
            return m.PreMult(v);
        }
        public static Vec4d operator *(Vec4d v, Matrixf m)
        {
            return m.PreMult(v);
        }
        public static Vec4f operator *(Matrixf lhs, Vec4f v)
        {
            return lhs.PostMult(v);
        }
        public static Vec4d operator *(Matrixf lhs, Vec4d v)
        {
            return lhs.PostMult(v);
        }
        public static Vec3f operator *(Matrixf lhs, Vec3f v)
        {
            return lhs.PostMult(v);
        }
        public static Vec3d operator *(Matrixf lhs, Vec3d v)
        {
            return lhs.PostMult(v);
        }
        public void SetRotate(Quat q)
        {
            ...
            {
                _mat[0, 0] = 0.0f; _mat[1, 0] = 0.0f; _mat[2, 0] = 0.0f;
                _mat[0, 1] = 0.0f; _mat[1, 1] = 0.0f; _mat[2, 1] = 0.0f;
                _mat[0, 2] = 0.0f; _mat[1, 2] = 0.0f; _mat[2, 2] = 0.0f;
            }
            else
            {
                double rlength2;
        ...
        public static Vec3f Transform3x3(Vec3f v, Matrixf m)
        {
            return new Vec3f((m._mat[0, 0] * v.X + m._mat[1, 0] * v.Y + m._mat[2, 0] * v.Z),
                         (m._mat[0, 1] * v.X + m._mat[1, 1] * v.Y + m._mat[2, 1] * v.Z),
                         (m._mat[0, 2] * v.X + m._mat[1, 2] * v.Y + m._mat[2, 2] * v.Z));
        }

        /** apply a 3x3 transform of v*M[0..2,0..2]. */
        public static Vec3d Transform3x3(Vec3d v, Matrixf m)
        {
        ...
        public static Vec3f Transform3x3(Matrixf m, Vec3f v)
        {
            return new Vec3f((m._mat[0, 0] * v.X + m._mat[0, 1] * v.Y + m._mat[0, 2] * v.Z),
                         (m._mat[1, 0] * v.X + m._mat[1, 1] * v.Y + m._mat[1, 2] * v.Z),
                         (m._mat[2, 0] * v.X + m._mat[2, 1] * v.Y + m._mat[2, 2] * v.Z));
        }

        /** apply a 3x3 transform of M[0..2,0..2]*v. */
        public static Vec3d Transform3x3(Matrixf m, Vec3d v)
        {
        ...
        public void Mult(Matrixf lhs, Matrixf rhs)
        {
            if (lhs == this)
            {
                PostMult(rhs);
        ...
        public void PreMult(Matrixf other)
        {
        ...
        public void PostMult(Matrixf other)
        {
            // brute force method requiring a copy
        ...
        public void PreMultTranslate(Vec3d v)
        {
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (tmp == 0)
                    continue;
                _mat[3, 0] += (float)(tmp * _mat[i, 0]);
                _mat[3, 1] += (float)(tmp * _mat[i, 1]);
                _mat[3, 2] += (float)(tmp * _mat[i, 2]);
                _mat[3, 3] += (float)(tmp * _mat[i, 3]);
            }
        }
        ...
        public void PostMultTranslate(Vec3d v)
        {
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (tmp == 0)
                    continue;
                _mat[0, i] += (float)(tmp * _mat[0, 3]);
                _mat[1, i] += (float)(tmp * _mat[1, 3]);
                _mat[2, i] += (float)(tmp * _mat[2, 3]);
                _mat[3, i] += (float)(tmp * _mat[3, 3]);
            }
        }
        ...
        {
            _mat[0, 0] *= (float)v[0]; _mat[0, 1] *= (float)v[0]; _mat[0, 2] *= (float)v[0]; _mat[0, 3] *= (float)v[0];
            _mat[1, 0] *= (float)v[1]; _mat[1, 1] *= (float)v[1]; _mat[1, 2] *= (float)v[1]; _mat[1, 3] *= (float)v[1];
            _mat[2, 0] *= (float)v[2]; _mat[2, 1] *= (float)v[2]; _mat[2, 2] *= (float)v[2]; _mat[2, 3] *= (float)v[2];
        }
        ...
        {
            _mat[0, 0] *= (float)v[0]; _mat[1, 0] *= (float)v[0]; _mat[2, 0] *= (float)v[0]; _mat[3, 0] *= (float)v[0];
            _mat[0, 1] *= (float)v[1]; _mat[1, 1] *= (float)v[1]; _mat[2, 1] *= (float)v[1]; _mat[3, 1] *= (float)v[1];
            _mat[0, 2] *= (float)v[2]; _mat[1, 2] *= (float)v[2]; _mat[2, 2] *= (float)v[2]; _mat[3, 2] *= (float)v[2];
        }
        ...
        public void Mult(Matrixf other)
        {
            if (this == other)
            {
                Matrixf temp = new Matrixf(other);
                PostMult(temp);
            }
            else PostMult(other);
        }

        public static Matrixf operator *(Matrixf l, Matrixf r)
        {
            r.Mult(l, r);
            return r;
        }
        ...
        /* Multiply by scalar. */
        public static Matrixf operator *(Matrixf lhs, value_type rhs)
        {
            return new Matrixf(
        ...
        public Matrixf Mult(value_type rhs)
        {
            _mat[0, 0] *= rhs;
            _mat[0, 1] *= rhs;
            _mat[0, 2] *= rhs;
        ...
        public Matrixf Div(value_type rhs)
        {
            _mat[0, 0] /= rhs;
            _mat[0, 1] /= rhs;
            _mat[0, 2] /= rhs;
        ...
        /** Binary vector add. */
        public static Matrixf operator +(Matrixf lhs, Matrixf rhs)
        {
            return new Matrixf(
        ...
        * intermediate object.
        */
        public Matrixf Add(Matrixf rhs)
        {
            _mat[0, 0] += rhs._mat[0, 0];
            _mat[0, 1] += rhs._mat[0, 1];
            _mat[0, 2] += rhs._mat[0, 2];
        ...
        private value_type InnerProduct(Matrixf a, Matrixf b, int r, int c)
        {
        ...
        public bool Equivalent(float lhs, float rhs, double epsilon = 1e-6)
        {
